...
이 가이드는 Amazon EKS에 확장 가능한 Apache Superset 플랫폼을 배포하는 방법을 설명합니다. 스택은 Terraform으로 프로비저닝되며 노드 자동 스케일링을 위한 Karpenter와 GitOps 스타일 애플리케이션 관리를 위한 ArgoCD가 포함됩니다.

### 아키텍처

이 스택은 다음 아키텍처를 프로비저닝합니다:
...
- **Apache Superset:** 비즈니스 인텔리전스 및 데이터 시각화 플랫폼.
- **Trino:** Superset이 S3의 데이터를 쿼리하는 데 사용하는 분산 SQL 쿼리 엔진.
- **ArgoCD:** GitOps 방식으로 Superset 및 Trino의 배포를 관리합니다.
- **Karpenter:** 필요할 때 새 EKS 노드를 자동으로 프로비저닝하여 클러스터가 효율적으로 확장될 수 있도록 합니다.
- **Amazon RDS:** Superset의 메타데이터 데이터베이스를 호스팅합니다.
- **Amazon S3:** Trino가 쿼리할 데이터를 저장합니다.
...
인프라가 배포되면 이제 모든 Superset 예제를 실행할 수 있습니다:

- [Superset DAG를 사용한 Spark 애플리케이션](/data-on-eks/docs/datastacks/databases/superset-on-eks/superset)
