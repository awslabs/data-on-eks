...
2. Prepare the S3 Bucket
    1. ``` cd ${DOEKS_HOME}/analytics/terraform/spark-k8s-operator/examples/karpenter/spark-mountpoint-s3/ ```
    2. ``` chmod +x prepare-s3.sh ```
    3. ``` ./prepare-s3.sh ```
3. Set-up Kubeconfig
    1. ```aws eks update-kubeconfig --name spark-operator-doeks```
4. Apply DaemonSet
    1. ```kubectl apply -f daemonset-with-configmap.yaml ```
4. Apply Spark Job sample
    1. 1. ```kubectl apply -f blueprints/spark-mountpoint-s3-3.4.0.yaml ```
4. View Job Running
