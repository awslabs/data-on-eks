...
Get the Argo Workflow WebUI url from the load balancer url, execute the command below:

```bash
URL=http://$(kubectl -n argo-workflows get service argo-workflows-server -o jsonpath="{.status.loadBalancer.ingress[*].hostname}{'\n'}"):2746
echo $URL
```

In your browser, access the WebUI via the output.
