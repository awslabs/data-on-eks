resource "helm_release" "volcano" {
  ...
  repository                 = try(var.volcano_helm_config["repository"], local.volcano_repository)
  chart                      = try(var.volcano_helm_config["chart"], "volcano-sh/${local.volcano_name}")
  version                    = try(var.volcano_helm_config["version"], local.volcano_version)
  ...
  namespace                  = try(var.volcano_helm_config["namespace"], local.volcano_name)
