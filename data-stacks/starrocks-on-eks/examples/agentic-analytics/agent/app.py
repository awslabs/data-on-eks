...
State machine: Decompose → Execute → Analyze → Summarize
  ...
  Decompose  — Bedrock writes 4-6 targeted SQL investigation queries
  Execute    — fans out concurrent read_query calls via a single shared MCP session
  Analyze    — Bedrock identifies the outlier dimension and value from results
  Summarize  — Bedrock synthesizes a concise markdown root-cause report
...
# Schema provided upfront so Bedrock can write correct StarRocks SQL
_SCHEMA = """
...
No markdown, no explanation - raw JSON array only."""
    ...
    log.info("[%s] Decompose %.1fs -> %d queries planned", rid, elapsed, len(plan))
    return {"plan": plan, "timing": {"decompose_s": elapsed}}


async def _run_query(mcp: MCPClient, item: dict, rid: str) -> dict:
    """Execute one read_query call on a shared MCP session."""
    sql = item.get("sql", "")
    t0  = time.time()
    rows_csv = ""
    error    = None
    try:
        ...
        log.warning("[%s] Query failed for [%s]: %s", rid, item.get("dimension_drilled"), exc)

    elapsed_ms = round((time.time() - t0) * 1000)
    log.info("[%s] read_query [%s] %dms - %d chars returned",
             rid, item.get("dimension_drilled"), elapsed_ms, len(rows_csv))
    return {
...
async def execute(state: InvestigationState) -> dict:
    rid = state.get("request_id", "?")
    log.info("[%s] === EXECUTE: %d concurrent MCP read_query calls ===", rid, len(state["plan"]))
    t0 = time.time()
    # Open one MCP session and fan out all queries concurrently over it.
    # This avoids N separate protocol handshakes for N queries.
    async with MCPClient(MCP_URL) as mcp:
        results = await asyncio.gather(
            *[_run_query(mcp, item, rid) for item in state["plan"]]
        )
    elapsed = round(time.time() - t0, 2)
    log.info("[%s] Execute %.1fs - all queries done", rid, elapsed)
    return {
...
One paragraph, no fluff."""
    ...
    log.info("[%s] Analyze %.1fs", rid, elapsed)
    return {
...
Under 300 words. Direct and actionable."""
    ...
    log.info("[%s] Report %.1fs", rid, elapsed)
    return {
        "report": report,
...
async def investigate(req: InvestigationRequest):
    ...
    log.info("[%s] Investigation started: %s (advertiser_id=%d)", rid, req.question, req.advertiser_id)
    t0 = time.time()
    try:
        ...
        log.exception("[%s] Investigation failed", rid)
        raise HTTPException(500, str(exc)) from exc

    timing = {**result.get("timing", {}), "total_s": round(time.time() - t0, 1)}
    log.info("[%s] Investigation complete in %.1fs", rid, timing["total_s"])
