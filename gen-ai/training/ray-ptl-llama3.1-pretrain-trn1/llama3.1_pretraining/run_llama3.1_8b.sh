...
Help()
{
   ...
   echo "g      global batch size"
   echo "z      mini batch size"
   echo "p      neuron parallel compile 0 or 1"
   echo
}
while getopts t:w:l:m:d:c:s:n:b:r:g:z:p:h flag
do
    case "${flag}" in
        t) MAX_STEPS=${OPTARG};;
...
EXTRA_ARGS=" "
if [ $PAR_COMP -gt 0 ]; then
    EXTRA_ARGS+=" --neuron_parallel_compile"
fi

DP=$(($NEURON_RT_NUM_CORES * $NUM_NODES / $TP_DEGREE))
ACC_STEPS=$(($GBS / $MBS / $DP))
echo NEURON_PARALLEL_COMPILE=$PAR_COMP
