...
aws glue delete-table --database-name $GLUE_DB_NAME --name $GLUE_TABLE_NAME --no-paginate
echo "Table deleted."
aws glue delete-crawler --name $CRAWLER_NAME --no-paginate
echo "Crawler deleted."
aws glue delete-database --name $GLUE_DB_NAME --no-paginate
