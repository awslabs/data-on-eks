...
EMR_VIRTUAL_CLUSTER_ID=$(terraform output -json emr_on_eks | jq -r '."test-team-a".virtual_cluster_id')
EMR_EXECUTION_ROLE_ARN=$(terraform output -json emr_on_eks | jq -r '."test-team-a".job_execution_role_arn')
CLOUDWATCH_LOG_GROUP=$(terraform output -json emr_on_eks | jq -r '."test-team-a".cloudwatch_log_group_name')
